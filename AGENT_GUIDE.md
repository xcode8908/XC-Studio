# XC Studio - Agent Guide

Start here. This is the complete operating guide and agent contract for XC Studio.
...
This is a first-class workflow in XC Studio.
...
If a model misses this distinction, it will often fall back to plain search + guesswork. That is incorrect for XC Studio.
...
XC Studio is an instruction-driven video production system. The AI agent IS the intelligence — it reads instructions (pipeline manifests + stage director skills + meta skills) and drives the pipeline using tools.
...
XC Studio uses two layers for capability choice:
...
XC Studio has three instruction layers:

1. `tools/`
   What exists, what is available, cost, runtime, fallback, related skills.
2. `skills/`
   How XC Studio wants those tools used in pipelines.
3. `.agents/skills/`
